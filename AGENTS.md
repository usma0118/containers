...
- Reviewer: `usma` — every PR requires human review and manual merge.
...
- All PRs must be reviewed by `usma` and merged manually.
...
- Reviewer `usma` must be assigned as code owner or reviewer in the repo to ensure PRs are reviewed promptly.
